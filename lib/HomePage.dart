...
const apiKey = "AIzaSyBKTgdL3Xl0uuWgiE4BxaJA9o8D_mnOcss";
...
class _TextOnlyState extends State<TextOnly> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color.fromARGB(234, 134, 192, 167),
        body: Column(
      children: [
        Expanded(
  ...
  void fromTextAndImage({required String query, required File image}) {
    ...
    gemini.generateFromTextAndImages(query: query, image: image).then((value) {
      setState(() {
        loading = false;
        textAndImageChat
            .add({"role": "GPT", "text": value.text, "image": ""});
      });
      scrollToTheEnd();
    }).onError((error, stackTrace) {
      setState(() {
        loading = false;
        textAndImageChat
            .add({"role": "GPT", "text": error.toString(), "image": ""});
      });
      scrollToTheEnd();
    });
  }
